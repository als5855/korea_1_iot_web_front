{
  ...
  // 전체사용자 데이터를 Users의 배열로 관리
  type Users = User[];

  //비동기적으로 사용자 데이터를 가져오는 함수
  // : async await 사용 + 화살표 함수를 사용한다.

  // 매개변수
  // - page: 현재 출력되는 페이지
  // - limit: 한 페이지 당 출력되는 사용자 뎅터의 개수 (3으로 고정)

  // 반환 타입: Promise<Users>
  // >> async awiat도 Promise를 기반으로 하기 때문에
  ...
  const fetchUsers = async (
    ...
    try {
      // JSONPlaceholder의 옵션
      // : _page 옵션 - 대량의 데이터를 호출할 때 특정 페이지의 데이터를 가져옴
      // :_limit 옵션 - 데이터를 조회할 때 한 번에 가져올 수 있는 함목의 최대 수를 지정
      // >> _page 옵션을 사용하는 경우 기본적으로 각 페이지에 한개의 데이터가 반환
  ...
  const createUserCard = (user: User): HTMLElement => {
    ...
    // const userCard = document.querySelcetor('.user-card');
    ...
    `;
    return userCard; // div태그 내에 h2, p 태그가 존재
  };
  ...
  const displayUsers = (users: Users): void => {
    ...
    if (userList) {
      userList.innerHTML = "";
      // users배열: 실재 출력될 데이터 배열 (3개의 요소를 포함)
      users.forEach((user) => {
        // 각 배열을 순회하면서 카드가 생성
  ...
  const loadPage = async (page: number): Promise<void> => {
    // 현재의 페이지값을전달하여 해당하는 데이터를 비동기적으로 가져옴
