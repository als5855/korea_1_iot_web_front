// ! DOM (문서객체 모델, Document Object Model)
...
// >> HTML의 모든 문서구조(내용)과 모드 로드된 이후에 이벤트 발생

// 문서 객체가 정립된 이수에 콜백함수(화살표 함수)가 실행
document.addEventListener('DOMContentLoaded', () => {
  // 해당 위치에 DOM조작 코드 작성하는 걸 권장한다.
});
...
// * 2-2 querySelcetor() & querySelcetorAll 
// : head 요속와 body 요소 내부에서 생성한다른 요소들을 읽어올 때 사용

// >> 선택자 부분에는 CSS 선택자를 입력
// Ex)태그, #아이디, .클래스, 선택자A 후손B(자식요소 선택자)

document.addEventListener('DOMContentLoaded', () => {
  // querySelctor 
  // :요소를 하나만 추출(여러 개일 경우 제일 상단의 요속만 가져오기)
